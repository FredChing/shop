...
class StatisticsController extends BaseController
{
    ...
    public function exportUserFinance(){
        ...
        $stat['member_count'] = count($memberList);
        $fields = ['tkmoney', 'sxfmoney', 'money' , 'all_order_count', 'paid_order_count', 'nopaid_order_count', 'all_order_amount', 'paid_order_amount', 'member_income', 'platform_income','balance','blockedbalance','pay_amount','pay_poundage'];
        foreach($fields as $field ) {
            foreach($memberList as $k => $v) {
                $stat[$field] += $v[$field];
            }
        ...
        foreach ($memberList as $m => $n){
            ...
            $list[] = array(
                ...
                'pay_amount'=>$n['paid_order_amount'],//实付金额
                'pay_poundage'=>$n['pay_poundage'],//入金手续费
                'balance'=>$n['balance'],
        ...
        $list[] = array(
            ...
            'pay_poundage'=>$stat['pay_poundage'].'元',//入金书序分
