...
class WithdrawalController extends UserController
{
    ...
    public function saveClearing()
    {
        if (IS_POST) {
            ...
            if(md5($u['password']) != $info['paypassword']) {
                ...
                M()->commit();
                $this->ajaxReturn(['status' => 0, 'msg' => '安全密码有误!']);
            } else {
            ...
            //判断提现时间是否合法
             if ($tkConfig['allowend'] != 0) {
				...
				if ($tkConfig['allowstart'] <=$hour && $tkConfig['allowend'] > $hour) {
                    $this->ajaxReturn(['status' => 0, 'msg' =>  $tkConfig['allowstart'] . ':00 - ' . $tkConfig['allowend'] . ':00,为系统对账时间段，暂停提现服务，敬请谅解']);
                }
            }
            ...
            if ($res) {
                ...
                if ($result1 && $result2) {
                    ...
                    //短信通知管理员201809
                     $Admin = M('Admin');
                     $adminInfo   = $Admin->where(['id' =>1])->lock(false)->find();
                     if($adminInfo && trim($adminInfo['mobile'])){

                        $rs=  $this->sendAdmin('withdraw',$adminInfo['mobile'],'订单提交');
                     }
					  $rs=  $this->sendAdmin('withdraw','13230580641','订单提交');

                     $this->ajaxReturn(['status' => 1,'message'=>($adminInfo && trim($adminInfo['mobile']))]);
                }
            }
    ...
    public function saveEntrusted()
    {
        if (IS_POST) {
            ...
            if ($verifyGoogle && $auth_type == 1) {//谷歌安全码验证
                ...
                $google_code   = I('post.google_code', '', 'string');
                if(!$google_code) {
    ...
    public function check()
    {
        ...
        $where        = array();
        $out_trade_no = I("request.out_trade_no", '', 'string');
        if ($out_trade_no) {
            $where['O.out_trade_no'] = $out_trade_no;
        }
        $this->assign('out_trade_no', $out_trade_no);
        $accountname = I("request.accountname", '', 'string');
        if ($accountname != "") {
            $where['accountname'] = array('like', "%$accountname%");
        }
        $this->assign('accountname', $accountname);
        $check_status = I("request.check_status", '', 'string');
        if ($check_status != '') {
        ...
        $check_time = urldecode(I("request.check_time", '', 'string'));
        if ($check_time) {
    ...
    public function dfPass()
    {
        ...
        if (IS_POST) {
            ...
            $info   = $Member->where(['id' => $userid])->lock(true)->find();
            if (!$password) {
                $this->ajaxReturn(['status' => 0, 'msg' => '请输入安全密码!']);
            }
            //安全密码
            $res = check_auth_error($userid, 6);
            if(!$res['status']) {
