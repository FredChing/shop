...
$data['CHANNELCODE'] = $_REQUEST["CHANNELCODE"];
$data['REMARK'] = "ALIPAY";
$data['RETURNURL'] = $_REQUEST['RETURNURL'];
$data['NOTIFYURL'] = 'http://www.taipay.net/Pay_DUYIQUICK_notifyurl.html';// $_REQUEST['NOTIFYURL'];
 
if ($data['ORDERNO'] == "" || $_REQUEST["TXNAMT"] == "" || $data['CHANNELCODE'] == ""  ) 
{
    echo "参数不能为空！";
    print_r($data);
    echo $_REQUEST["ORDERNO"]. $_REQUEST["TXNAMT"].$_REQUEST['CHANNELCODE'];
    exit();
}
 
 
  
$TRANDATA = "ORDERNO=" . $data['ORDERNO'] . "&TXNAMT=" . $data['TXNAMT']. "&RETURNURL=" . $data['RETURNURL'] . "&NOTIFYURL=" . $data['NOTIFYURL'] . "&CHANNELCODE=" . $data['CHANNELCODE'] . "&REMARK=" . $data['REMARK'] ;

$pi_key = openssl_pkey_get_private($private);
openssl_sign($TRANDATA, $SIGN, $pi_key, OPENSSL_ALGO_SHA1);

$SIGN = base64_encode($SIGN);
$gateway = 'http://111.231.217.165:28080/Pay/trans/h5PayOrder';
