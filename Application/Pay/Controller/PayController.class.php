...
class PayController extends Controller
{
  ...
  {
      if(stristr($_SERVER['HTTP_VIA'],"wap")){return true;}elseif(strpos(strtoupper($_SERVER['HTTP_ACCEPT']),"VND.WAP.WML") > 0){return true;}
  ...
  public function getSignkey( $code, $merid )
	{
		$channel = M('channel');
        $result = $channel -> join('LEFT JOIN pay_channel_account ON pay_channel.id = pay_channel_account.channel_id') -> where( 'pay_channel.code = "'.$code.'" and pay_channel_account.mch_id ="'.$merid.'" ' )->find();
    ...
    public function orderadd($parameter)
    {
        ...
        //201809
        $site=trim($unlockdomain)?$unlockdomain."/":$this->_site;
        //回调参数
        $return = [
            ...
            "notifyurl"    => $syschannel["serverreturn"] ? $syschannel["serverreturn"] :  $site . "Pay_" .
            $PayName . "_notifyurl.html",
            "callbackurl"  => $syschannel["pagereturn"] ? $syschannel["pagereturn"] :  $site. "Pay_" .
        ...
        if ($this->verify()) {
            ...
            $data['qrcode_id']           = $parameter["qrcode_id"];
            $data['pay_type']            = I('request.pay_type', '0');
            ...
            if ($Order->add($data)) {
                ...
                $return['pay_applydate'] = $data['pay_applydate'];
    ...
    {
        Log::record('回调处理订单EditMoney():' . $trans_id);
        $m_Order    = M("Order");
        ...
        switch ($returntype) {
            case '0':
                ...
                curl_close($ch);
                Log::record('真：回调来了+++++++' . $order_info["pay_orderid"]);
   ...
   protected function EditMoney2($trans_id, $pay_name = '', $returntype = 1, $transaction_id = '')
    {
        ...
        if ($order_info["pay_status"] == 0)
        {
           ...
           $ymoney                 = $member_info['balance']; //改动前的金额
           $gmoney                 = sprintf("%.4f", $member_info['balance'] -  $order_info['pay_poundage'] ) ; //改动后的金额
           $member_data = [
             'balance' => $member_info['balance']  -1 * $order_info['pay_poundage']
             ];
            ...
            $moneychange_data = [
                'userid'     => $userid,
                'ymoney'     => $ymoney, //原金额或原冻结资金
                'money'      =>  -1 * $order_info['pay_poundage'],
                'gmoney'     => $gmoney, //改动后的金额或冻结资金
                'datetime'   => date('Y-m-d H:i:s'),
                'tongdao'    => $order_info['pay_bankcode'],
                'transid'    => $trans_id,
                'orderid'    => $order_info['out_trade_id'],
                'contentstr' => '订单 '.$order_info['out_trade_id'] . ' 支付扣除手续费',
                'lx'         => 20,
                't'          => $order_info['t']
            ];
...
public function checkorder()
    ...
    public function checkstatus()
    {
        ...
        if ($order['pay_status'] != 0) {
            echo json_encode(array('status' => 'ok', 'callback' => $this->_site . "Pay_" . $order['pay_tongdao'] . "_callbackurl.html?orderid="
                . $orderid . "&pay_memberid=" . $order['pay_memberid'] . '&bankcode=' . $order['pay_bankcode']. '&callbackurl=' . $order['pay_callbackurl']));
            exit();
        } else {
            exit("no-$orderid");
        }
    ...
    {
        header('Content-Type:application/json; charset=utf-8');
        $data = array('status' => 'ok', 'msg' => $msg, 'data' => $fields);
