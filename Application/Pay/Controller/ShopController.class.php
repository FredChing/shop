...
class ShopController extends PayController
{
    ...
    public function Pay($array)
    {
        ...
        $notifyurl = $this->_site . 'Pay_Shop_notifyurl.html'; //异步通知
        $callbackurl = $this->_site . 'Pay_Shop_callbackurl.html'; //跳转通知
        $parameter = array(
            'code'         => 'Shop',     // 通道代码
            'title'        => '商城',     //通道名称
            'exchange'     => 100,        // 金额比例
         ...
         */
        //组装请求参数、并发起请求
        $this->setHtml($return['gateway'], $return);
    }
    ...
    {
        $orderid = $_GET['out_trade_no']; //系统订单号
        //处理验签
        // .............................. 省略部分代码
        //生成签文，开始验签
        $verify = true;
        if ($verify) {
            ...
            if ($pay_status <> 0) {
                //业务逻辑开始、并下发通知.
                $this->EditMoney($orderid, 'Shop', 1);
            } else {
                $succ = $_GET['succ'];
                if ($succ == 1) {
                    $order_data['pay_status'] = 1;
                    $Order->where(['pay_orderid' => $orderid])->save($order_data);
                    $this->EditMoney($orderid, 'Shop', 1);
                } else {
                    exit('error');
                }
    ...
    {
        Log::record('回调来了+++++++' . $_POST['out_trade_no']);
        //系统订单号
