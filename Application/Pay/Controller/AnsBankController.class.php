...
class AnsBankController extends PayController
{
    ...
    public function Pay($array)
    {
        ...
        if($return['unlockdomain']) {//防封域名
            $gateway = $return['unlockdomain'].'/Pay_AnsBank_Rpay';
            $frontUrl = $return['unlockdomain'].'/Pay_AnsBank_callbackurl.html';
            $backUrl = $return['unlockdomain'].'/Pay_AnsBank_notifyurl.html';
        } else {
            $gateway = $this->_site.'Pay_AnsBank_Rpay';
            $frontUrl = $this->_site.'Pay_AnsBank_callbackurl.html';
            $backUrl = $this->_site.'Pay_AnsBank_notifyurl.html';
        }
        $formData = [
    ...
    public function notifyurl()
    {
        ...
        $signValue = $this->md5Sign($_POST, $key);
